    ...
    buildMapping(validateValue) {
      ...
      if (state.ch !== '{') {
        ...
        let {
          value: val,
          state: nextState,
        } = object(state, (key, valueState, keyState) => {
          ...
          let keyContext = new Context(keyState, `While validating key "${key}"`, this);
  ...
  var escapee = {
    ...
    error = function(m, state) {
      ...
      error.message =
        ...
        JSON.stringify(state.text.substring(state.at - 1, state.at + 19));
      error.at = state.at;
