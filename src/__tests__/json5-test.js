...
describe('validated/json5', function() {
  describe('constant', function() {
    ...
    itDoesNotValidate('not-ok', schema, '"not-ok"', {
      format: ['Expected "ok" but got "not-ok" (line 1 column 1)'].join('\n'),
    });
    itDoesNotValidate('1', schema, '1', {
      format: ['Expected "ok" but got 1 (line 1 column 1)'].join('\n'),
    });
  });
  ...
  describe('string', function() {
    ...
    itDoesNotValidate('number', schema, '1', {
      format: ['Expected value of type string but got number (line 1 column 1)'].join(
        '\n',
      ),
    });
    itDoesNotValidate('number', schema, '  1', {
      format: ['Expected value of type string but got number (line 1 column 3)'].join(
        '\n',
      ),
    });
    itDoesNotValidate('number', schema, '  \n1', {
      format: ['Expected value of type string but got number (line 2 column 0)'].join(
        '\n',
      ),
    });
  });
  ...
  describe('number', function() {
    ...
    itDoesNotValidate('string', schema, '"ok"', {
      format: ['Expected value of type number but got string (line 1 column 1)'].join(
        '\n',
      ),
    });
  });
  ...
  describe('boolean', function() {
    ...
    itDoesNotValidate('string', schema, '"ok"', {
      format: ['Expected value of type boolean but got string (line 1 column 1)'].join(
        '\n',
      ),
    });
  });
  ...
  describe('mapping', function() {
    describe('simple schema', function() {
      ...
      itDoesNotValidate('[]', schema, '[]', {
        format: ['Expected a mapping but got array (line 1 column 1)'].join('\n'),
      });
    });
  ...
  describe('object', function() {
    describe('with fields', function() {
      ...
      itDoesNotValidate('Number', schema, '1', {
        format: ['Expected a mapping but got number (line 1 column 1)'].join('\n'),
      });
      itDoesNotValidate('Boolean', schema, 'true', {
        format: ['Expected a mapping but got boolean (line 1 column 1)'].join('\n'),
      });
      itDoesNotValidate('String', schema, "'not ok'", {
        format: ['Expected a mapping but got string (line 1 column 1)'].join('\n'),
      });
    });
  ...
  describe('sequence', function() {
    describe('simple schema', function() {
      ...
      itDoesNotValidate('Object', schema, '{}', {
        format: ['Expected an array but got object (line 1 column 1)'].join('\n'),
      });
      itDoesNotValidate('Number', schema, '1');
      itDoesNotValidate('Boolean', schema, 'true');
      itDoesNotValidate('String', schema, "'not ok'");
  ...
  describe('oneOf', function() {
    ...
    describe('with containers (nested complex assymmetrical)', function() {
      let schema = oneOf(
        object({a: number}),
        object({a: oneOf(object({c: number}), boolean)}),
      );
    ...
    describe('with containers (nested complex symmetrical)', function() {
      let schema = oneOf(
        object({a: oneOf(object({c: boolean}), number)}),
        object({a: oneOf(object({c: number}), boolean)}),
      );
  ...
  describe('enumeration', function() {
    ...
    itValidates('"ok"', schema, "'ok'", 'ok');
    itDoesNotValidate('1', schema, '1', {
      format: ['Expected value to be one of 42, "ok" but got 1 (line 1 column 1)'].join(
        '\n',
      ),
    });
  });
